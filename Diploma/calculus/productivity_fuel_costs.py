...
def test_first_phase_of_calculations(aircraft_info, l_i, g_p_c, country="UKR"):
    ...
    a_ftk_max = np.round(calculate_aircraft_productivity_per_flight(g_pi=g_pi, v_f=v_f), ROUND_TO)
    a_ftk_c = np.round(calculate_aircraft_productivity_per_flight_at_calculated_payload(l_i=l_i, l_ult=l_ult, g_p_c=g_p_c,
    ...
    Q_f = np.round(calculate_fuel_burn_per_flight(q_f=q_f, q_cr_km=q_cr_km, l_cruise=l_cruise), ROUND_TO)

    q_sp_1 = np.round(
        calculate_specific_fuel_consumption_at_max_payload(Q_f=Q_f, g_p_max=g_p_max, l_i=l_i, l_ult=l_ult, g_pi=g_pi),
        ROUND_TO)
    q_sp_2 = np.round(
        calculate_specific_fuel_consumption_at_calculated_payload(Q_f=Q_f, g_p_c=g_p_c, l_i=l_i, l_ult=l_ult, e_c=e_c,
                                                                  g_pi=g_pi), ROUND_TO)
    ...
    c_f = np.round(calculate_flight_costs(c_fh=c_fh, t_f=t_f, Q_f=Q_f, C_t=C_t), ROUND_TO)

    c_sp_1 = np.round(calculate_specific_transportation_costs_at_max_payload(c_f=c_f, g_p_max=g_p_max, g_pi=g_pi, l_i=l_i,
                                                                          l_ult=l_ult), ROUND_TO)
    c_sp_2 = np.round(calculate_specific_transportation_costs_at_calculated_payload(c_f=c_f, g_p_c=g_p_c, l_i=l_i),
                   ROUND_TO)

    return a_ftk_c, np.round(a_ftk_c / a_ftk_max, ROUND_TO), Q_f, \
           np.round(q_sp_2 / q_sp_1, ROUND_TO), \
           c_f, c_sp_2, np.round(c_sp_2 / c_sp_1, ROUND_TO)
